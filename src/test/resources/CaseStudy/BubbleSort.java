...
class BubbleSort {
    /*@
      ...
      @ ensures (\forall int v; 0 <= v && v < \result.length; (\forall int w; 0 <= w && w < v; \result[w] <= \result[v]));
      @ assignable arr[*];
      @*/
    static int[] sort(int arr[]) {
        //@ loop_invariant j <= arr.length - 1 && -1 <= j;
        //@ loop_invariant (\forall int k; j + 1  <= k && k < arr.length; (\forall int k1; 0 <= k1 && k1 < k; arr[k1] <= arr[k]));
        //@ assignable arr[*];
        //@ decreases j + 1;
