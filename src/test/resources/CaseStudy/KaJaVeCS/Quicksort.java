...
public class Quicksort {
    ...
    /*+Key@ public normal_behaviour
      ...
      @  measured_by to - from + 1, 1;
      @*/
    private static void sort(int[] array, int from, int to) {
        if (from >= to) return;

        if (to - from <= BaseSort.THRESHOLD) {
            BaseSort.baseSort(array, from, to);
            return;
        }
    ...
    /*+Key@ public normal_behaviour
      @  requires 0 <= from < to < array.length;
      @  requires from > 0 ==> (\forall int x; from<=x && x<=to; array[x] > array[from-1]);
      @  requires to < array.length-1 ==> (\forall int x; from<=x && x<=to; array[x] <= array[to+1]);
      @  ensures \dl_seqPerm(\dl_array2seq(array), \old(\dl_array2seq(array)));
      @  ensures (\forall int i; from<=i && i<to; array[i] <= array[i+1]);
  ...
  @*/
    /*+OPENJML@ private normal_behaviour
       @  requires array != null;
  @  requires 0 <= a < array.length;
  @  requires 0 <= b < array.length;
  @*/
    private static void swap(int[] array, int a, int b) {
        ...
        //+OPENJML@ assert array[b] == \old(array[a]) && array[a] == \old(array[b]);
        //+KEY@ assume array[b] == \old(array[a]) && array[a] == \old(array[b]);
    }
}